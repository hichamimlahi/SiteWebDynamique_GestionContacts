...
if (isset($_POST['submit'])) {
    ...
    $confirmer_mot_de_pass = $_POST['confirmer_mot_de_pass'];
    ...
    if ($mot_de_pass !== $confirmer_mot_de_pass) {
        ...
        $sql_check = 'SELECT COUNT(*) FROM utilisateurs WHERE nom_utilisateur = :nom_utilisateur';
        $statement_check = $connection->prepare($sql_check);
        $statement_check->execute([':nom_utilisateur' => $nom_utilisateur]);
        if ($statement_check->fetchColumn() > 0) {
            ...
            $sql_insert = 'INSERT INTO utilisateurs (nom_utilisateur, mot_de_pass) VALUES (:nom_utilisateur, :mot_de_pass)';
            $statement_insert = $connection->prepare($sql_insert);
            if ($statement_insert->execute([':nom_utilisateur' => $nom_utilisateur, ':mot_de_pass' => $hashed_password])) {
                $message = '<div class="alert alert-success">Compte créé avec succès ! Vous pouvez maintenant vous <a href="login.php">connecter</a>.</div>';
            } else {
...
<html lang="fr">
<head>
    <title>Inscription - Carnet de Contacts</title>
    <meta charset="utf-8">
    ...
    <div class="card p-4">
        ...
        <div class="card-body">
            ...
            <form method="post">
                ...
                <div class="mb-3">
                    <label for="confirmer_mot_de_pass" class="form-label">Confirmer le mot de passe</label>
                    <div class="input-group">
                        <span class="input-group-text"><i class="fas fa-lock"></i></span>
                        <input type="password" name="confirmer_mot_de_pass" id="confirmer_mot_de_pass" class="form-control" required>
                    </div>
                </div>
                <div class="d-grid mt-4">
                    <button type="submit" name="submit" class="btn btn-primary"><i class="fas fa-user-plus me-2"></i>S'inscrire</button>
                </div>
